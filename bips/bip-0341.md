...
softfork, users SHOULD NOT include `annex` in transactions, or it
may lead to PERMANENT FUND LOSS.
...
approximately equal to *log~2~(1/probability)*, but to have branches
longer than 128 you would need to have scripts with an execution
chance below 1 in *2^128^*. As that is our security bound, scripts
that truly have such a low chance can probably be removed entirely.

[^7]: **What constraints are there on the leaf version?** First, the
leaf version cannot be odd as *c\[0\] & 0xfe* will always be even,
and cannot be *0x50* as that would result in ambiguity with the
...
*v* and *v \| 1* should be an undefined, invalid or disabled opcode
...
tree?** By doing so, it is not necessary to reveal the left/right
...
is required to enable future leaf versions as soft forks
...
\`sha_prevouts\` if `SIGHASH_ANYONECANPAY` is set) instead of
...
the data (implicit in *hash_type* and *spend_type*) before the
...
if `SIGHASH_SINGLE` or `SIGHASH_NONE` are set?** Because setting
them already makes the message commit to the `prevouts` part of all
...
*hash~TapSighash~* tagged hash in future signature algorithms that
make invasive changes to how hashing is performed (as opposed to the
*ext_flag* mechanism that is used for incremental extensions). An
alternative is having them use a different tag, but supporting a
growing number of tags may become undesirable.
