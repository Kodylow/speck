...
### Relation to BIP79 (Bustapay) {#relation_to_bip79_bustapay}
...
### BIP21 payjoin parameters {#bip21_payjoin_parameters}
...
### `<span id="optional-params">`{=html}`</span>`{=html}Optional parameters {#optional_parameters}
...
### Receiver\'s well known errors {#receivers_well_known_errors}
...
### `<span id="fee-output">`{=html}`</span>`{=html}Fee output {#fee_output}
...
### Receiver\'s original PSBT checklist {#receivers_original_psbt_checklist}
...
### Sender\'s payjoin proposal checklist {#senders_payjoin_proposal_checklist}
...
### Respecting the minimum relay fee policy {#respecting_the_minimum_relay_fee_policy}
...
### Defeating heuristics based on the fee calculation {#defeating_heuristics_based_on_the_fee_calculation}
...
### Receiver does not need to be a full node {#receiver_does_not_need_to_be_a_full_node}
...
### `<span id="spare-change">`{=html}`</span>`{=html}Spare change donation {#spare_change_donation}
...
### `<span id="output-substitution">`{=html}`</span>`{=html}Payment output substitution {#payment_output_substitution}
...
### `<span id="unsecured-payjoin">`{=html}`</span>`{=html}Unsecured payjoin server {#unsecured_payjoin_server}
...
### Impacted heuristics {#impacted_heuristics}
...
### `<span id="probing-attack">`{=html}`</span>`{=html}On the receiver side: UTXO probing attack {#on_the_receiver_side_utxo_probing_attack}
...
### On the sender side: Double payment risk for hardware wallets {#on_the_sender_side_double_payment_risk_for_hardware_wallets}
...
## `<span id="reference-impl">`{=html}`</span>`{=html}Reference sender\'s implementation {#reference_senders_implementation}
...
## `<span id="test-vectors">`{=html}`</span>`{=html}Test vectors {#test_vectors}
